...
export default function PayerTab() {
  ...
  const savePayer = async () => {
    try {
      if (editingId) {
        await api.patch(`/finance/payer/${editingId}`, form);
      } else {
        await api.post("/finance/payer", form);
      }
  ...
  const deletePayer = async (id) => {
    ...
    await api.delete(`/finance/payer/${id}`);
    fetchPayers();
  };
  ...
  const openEdit = (p) => {
    setForm({
      ...
      vendor_type: p.vendor_type || "Customer",
      email: p.email || "",
  ...
  return (
    <>
      ...
      {/* MODAL */}
      {showModal && (
        <Modal title="Add Payer" onClose={() => setShowModal(false)}>
          <div className="grid grid-cols-3 gap-4">
            ...
            <div>
              <label className="soft-label">
                Display Payer Name <span className="text-red-500">*</span>
              </label>
              <input
